...
REPLICA_NAME="${REPLICA_NAME:-replica_1}"
...
if [ -z "$(ls -A /var/lib/postgresql/data 2>/dev/null)" ]; then
  ...
  # Set application_name for synchronous replication tracking
  echo "primary_conninfo = 'host=$PRIMARY_HOST port=5432 user=$REPLICATION_USER password=$REPLICATION_PASSWORD application_name=$REPLICA_NAME sslmode=prefer'" \
